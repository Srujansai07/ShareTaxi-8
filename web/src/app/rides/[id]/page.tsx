...
import { getRideDetails, joinRide } from '@/app/actions/rides'
import { getMatches } from '@/app/actions/rides'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import { MatchCard } from '@/components/MatchCard'
import { MapPin, Clock, Users, DollarSign, MessageCircle, Navigation, AlertCircle, Car } from 'lucide-react'
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="h-16 w-16 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-semibold text-xl">
                                        {ride.user?.photoUrl ? (
                                            ...
                                            getInitials(ride.user?.displayName || 'User')
                                        )}
                                    </div>

                                    <div>
                                        <CardTitle>{ride.user?.displayName || 'User'}</CardTitle>
                                        <CardDescription>
                                            ⭐ {ride.user?.trustScore?.toFixed(1) || '5.0'} • {ride.user?.totalRides || 0} rides
                                        </CardDescription>
                                    </div>
                                </div>
                                            ...
                                            <div key={participant.id} className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                                                <div className="h-10 w-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-semibold">
                                                    {participant.user?.photoUrl ? (
                                                        ...
                                                        getInitials(participant.user?.displayName || 'U')
                                                    )}
                                                </div>
                                                <div className="flex-1">
                                                    <p className="font-semibold">{participant.user?.displayName}</p>
                                                    <p className="text-xs text-muted-foreground">
                                                        ⭐ {participant.user?.trustScore?.toFixed(1) || '5.0'} • {participant.role}
                                                    </p>
                                                </div>
                ...
                <div className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                {matches?.length || 0} neighbors going the same way
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
